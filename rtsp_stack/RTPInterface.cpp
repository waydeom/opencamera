...
int RTPInterface::sendPacket(unsigned char* packet, unsigned packetSize) {
  ...
  int tcpSendIsSucess = 0;
  fGS->output(envir(), fGS->ttl(), packet, packetSize);

  // Also, send over each of our TCP sockets:
  for (tcpStreamRecord* streams = fTCPStreams; streams != NULL;
       streams = streams->fNext) {
    tcpSendIsSucess = sendRTPOverTCP(packet, packetSize,
		   streams->fStreamSocketNum, streams->fStreamChannelId);
	Debug(ckite_log_message, "Send packet %d bytes, %x %x %x %x\n", packetSize, packet[12], packet[13], packet[14], packet[15]);
  }

  return tcpSendIsSucess;
}
...
int sendRTPOverTCP(unsigned char* packet, unsigned packetSize,
                    int socketNum, unsigned char streamChannelId) {
#if 1//def DEBUG
  Debug(ckite_log_message, "sendRTPOverTCP: %d bytes over channel %d (socket %d)\n", \
	  packetSize, streamChannelId, socketNum); fflush(stderr);
#endif
  // Send RTP over TCP, using the encoding defined in
  // RFC 2326, section 10.12:
  struct timeval timeout = {10,0};  // add by wayde
  struct timeval timeout1 = {0,0};
  setsockopt(socketNum,SOL_SOCKET,SO_SNDTIMEO,(char *)&timeout,sizeof(struct timeval));
  int ret = 0;
  do {
    char const dollar = '$';
    if (ret = send(socketNum, &dollar, 1, 0) != 1) break;
    if (ret = send(socketNum, (char*)&streamChannelId, 1, 0) != 1) break;

    char netPacketSize[2];
    netPacketSize[0] = (char) ((packetSize&0xFF00)>>8);
    netPacketSize[1] = (char) (packetSize&0xFF);
    if (ret = send(socketNum, netPacketSize, 2, 0) != 2) break;

    if (ret = send(socketNum, (char*)packet, packetSize, 0) != (int)packetSize) break;

#ifdef DEBUG
    Debug(ckite_log_message, "sendRTPOverTCP: completed\n"); fflush(stderr);
#endif
    return 1;
  } while (0);

  Debug(ckite_log_message, "sendRTPOverTcp ret = %d\n", ret);
  if (ret == -1)
  {
	  if (errno == EINTR)
	  {
		  // socket buffer block, try again
		  return 0;
	  }
	  else
	  {
  ...
  return 0;
#ifdef DEBUG
  Debug(ckite_log_message, "sendRTPOverTCP: failed!\n"); fflush(stderr);
#endif
}
...
#ifdef DEBUG
    Debug(ckite_log_message, "SocketDescriptor::tcpReadHandler() reading %d bytes on channel %d\n", rtpInterface->fNextTCPReadSize, streamChannelId);
#endif
