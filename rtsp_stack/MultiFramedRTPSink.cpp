...
MultiFramedRTPSink::MultiFramedRTPSink(UsageEnvironment& env,
  ...
  fOutBuf(NULL), fOwner(owner), fCurFragmentationOffset(0), fPreviousFrameEndedFragmentation(False) {
...
void MultiFramedRTPSink::buildAndSendPacket(Boolean isFirstPacket) {
  ...
  //Debug(ckite_log_message, "MultiFramedRTPSink::buildAndSendPacket ssrc = %x\n", SSRC());
...
void MultiFramedRTPSink::sendPacketIfNecessary() {
  ...
  if (fOutBuf->haveOverflowData()
	...
	fOutBuf->resetOffset(); //wayde comment
	fNumFramesUsedSoFar = 0;
...
void MultiFramedRTPSink::sendNext(void* firstArg) {
  //Debug(ckite_log_message, "::sendNext\n");
