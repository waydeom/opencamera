...
ServerMediaSession::addSubsession(ServerMediaSubsession* subsession) {
  ...
  Debug(ckite_log_message, "addSubsession subsession = %x, head=%x, tail=%x\n", subsession, fSubsessionsHead, fSubsessionsTail);
...
void ServerMediaSession::resetMediaServersession(void) {
	//Medium::close(fSubsessionsHead);
	//delete[] fStreamName;
	//delete[] fInfoSDPString;
	//delete[] fDescriptionSDPString;
...
void ServerMediaSubsession::resetServerMediaSubsession(){
	delete[] (char *)fTrackId;
	Debug(ckite_log_message, "resetServerMediaSubsession fNext = %x\n", fNext);
	Medium::close(fNext);
}

ServerMediaSubsession::~ServerMediaSubsession() {
  Debug(ckite_log_message, "ServerMediaSubsession::~ServerMediaSubsession\n");
  delete[] (char*)fTrackId;
  Debug(ckite_log_message, "fNext = %x\n", fNext);
  Medium::close(fNext);
}
...
char const* ServerMediaSubsession::trackId() {
  ...
  if (fTrackId == NULL) {
    char buf[100];
    sprintf(buf, "trackID=%d", fTrackNumber);
    fTrackId = strDup(buf);
  }
  return fTrackId;
}
