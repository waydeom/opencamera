...
void BasicTaskScheduler::SingleStep(unsigned maxDelayTime) {
...
#ifndef __WIN32__
  //fprintf(stderr, "fd_sets = %x %x %x\n", __FDS_BITS(&readSet)[0], __FDS_BITS(&writeSet)[0], __FDS_BITS(&exceptionSet)[0]);
#endif
 ...
 //     fprintf(stderr, "Event %x of sock %d triggerred, handler %x\n", resultConditionSet, sock, (int)handler->handlerProc);
...
void BasicTaskScheduler
  ...
  if (conditionSet == 0) {
	...
	fprintf(stderr, "fd_set socket: %d\n", socketNum);
#ifndef __WIN32__
    //fprintf(stderr, "setBackgroundHandling: fd_sets = %x %x %x\n", __FDS_BITS(&fReadSet)[0], __FDS_BITS(&fWriteSet)[0], __FDS_BITS(&fExceptionSet)[0]);
#endif
  }
}
...
void BasicTaskScheduler::moveSocketHandling(int oldSocketNum, int newSocketNum) {
  ...
  fprintf(stderr, "fd_set new socket: %d\n", newSocketNum);
