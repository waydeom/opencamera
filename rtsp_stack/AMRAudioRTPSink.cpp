...
AMRAudioRTPSink*
AMRAudioRTPSink::createNew(UsageEnvironment& env, void* owner, Groupsock* RTPgs,
			   unsigned char rtpPayloadFormat,
			   Boolean sourceIsWideband,
			   unsigned numChannelsInSource) {
  return new AMRAudioRTPSink(env, owner, RTPgs, rtpPayloadFormat,
			     sourceIsWideband, numChannelsInSource);
}

AMRAudioRTPSink
::AMRAudioRTPSink(UsageEnvironment& env, void* owner, Groupsock* RTPgs,
		  unsigned char rtpPayloadFormat,
		  Boolean sourceIsWideband, unsigned numChannelsInSource)
  : AudioRTPSink(env, owner, RTPgs, rtpPayloadFormat,
...
void AMRAudioRTPSink::doSpecialFrameHandling(unsigned fragmentationOffset,
  ...
  toc &=~ 0x80;
  if (numFramesUsedSoFar() != specialHeaderSize() - 2)
	  toc |= 0x80;
