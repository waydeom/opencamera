...
RTPSink::RTPSink(UsageEnvironment& env,
  ...
  fSeqNo = 0;//(u_int16_t)our_random();
  fSSRC = our_random32();
  fTimestampBase = 0;//our_random32();
...
u_int32_t RTPSink::convertToRTPTimestamp(struct timeval tv) {
  ...
  Debug(ckite_log_message, "fTimestampBase: 0x%08x, tv: %lu.%06ld\n\t=> RTP timestamp: 0x%08x\n",
...
char const* RTPSink::auxSDPLine() {
	Debug(ckite_log_message, "auxSDPLine Entry\n");
...
::noteIncomingRR(u_int32_t SSRC, struct sockaddr_in const& lastFromAddress,
  ...
  if (stats == NULL) {
    ...
    add(SSRC, stats);
	Debug(ckite_log_message, "RTPTransmissionStatsDB::noteIncomingRR ssrc = %x\n", SSRC);
#ifdef DEBUG_RR
    Debug(ckite_log_message, "Adding new entry for SSRC %x in RTPTransmissionStatsDB\n", SSRC);
#endif
  }
...
::noteIncomingRR(struct sockaddr_in const& lastFromAddress,
...
#ifdef DEBUG_RR
  Debug(ckite_log_message, "RTCP RR data (received at %lu.%06ld): lossStats 0x%08x, lastPacketNumReceived 0x%08x, jitter 0x%08x, lastSRTime 0x%08x, diffSR_RRTime 0x%08x\n",
  ...
  Debug(ckite_log_message, "=> round-trip delay: 0x%04x (== %f seconds)\n", rtd, rtd/65536.0);
#endif
